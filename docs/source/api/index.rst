...
session or API key, and a global limit of 90 requests every 80 seconds per account.
Rate limits are configuration settings specific to a site, so these values may not
be consistent with values on other sites.
