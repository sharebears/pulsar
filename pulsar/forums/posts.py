...
def view_post(id: int) -> flask.Response:
    ...
    **Example response**:
    ...
    """
    post = ForumPost.from_id(id, _404='Forum post')
...
def create_post(contents: str,
    ...
    **Example response**:
    ...
    """
    thread = ForumThread.from_id(thread_id, _404='Forum thread')
...
def modify_post(id: int,
    ...
    **Example response**:
    ...
    if not thread:
        raise APIException(f'{ForumPost.__name__} {id} does not exist.')
...
def delete_post(id: int) -> flask.Response:
    ...
    **Example response**:
    ...
    post.deleted = True
    db.session.commit()
    return flask.jsonify(f'Forum post {id} has been deleted.')
